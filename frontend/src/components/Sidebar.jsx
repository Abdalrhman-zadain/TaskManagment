...
export default function Sidebar({ role }) {
  ...
  return (
    <aside className="w-52 bg-white border-r border-slate-200 flex flex-col pt-6 pb-4 flex-shrink-0 h-screen sticky top-0">
      {/* Logo */}
      <div className="px-5 pb-6 border-b border-slate-200 mb-5 shrink-0">
        <div className="font-bold text-xl tracking-tight text-slate-900">
          Team<span className="text-blue-400">Task</span>
        </div>
        <div className="text-xs text-slate-500 mt-0.5">{role} Portal</div>
      </div>
      ...
      <div className="mt-auto pt-4 px-4 border-t border-slate-200 shrink-0">
        ...
        <button
          onClick={logout}
          className="w-full text-xs text-slate-500 hover:text-red-500 text-left transition py-1"
        >
          Sign out →
        </button>
      </div>
