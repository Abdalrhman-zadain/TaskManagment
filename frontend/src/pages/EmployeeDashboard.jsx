    ...
    <div className="flex min-h-screen bg-[#0F1D3A]">
      ...
      <main className="flex-1 p-7 overflow-y-auto">
        ...
        <div className="mb-7">
          ...
          <p className="text-sm text-slate-400 mt-0.5">{new Date().toDateString()}</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div className="bg-white/4 border border-white/8 rounded-xl p-4">
            <h2 className="text-sm font-bold mb-3">Active Tasks ({active.length})</h2>
            {active.length === 0
              ? <p className="text-sm text-slate-400 py-4 text-center">No active tasks 🎉</p>
              : active.map(task => <TaskCard key={task.id} task={task} onMarkDone={markDone} />)
            }
          </div>

          {/* Completed Tasks */}
          <div className="bg-white/4 border border-white/8 rounded-xl p-4">
            <h2 className="text-sm font-bold mb-3">Completed ({done.length})</h2>
            {done.length === 0
              ? <p className="text-sm text-slate-400 py-4 text-center">No completed tasks yet</p>
